...
| 角色增删改查 / 菜单分配 / 用户分配角色 | 管理端页面 + `system:role:*`、`system:user:*` 端点；本轮补充内置编码防提权 | 编译通过，逻辑已审查 | 09-24 |
...
| 本轮 4 项快修（角色防提权/SCAN/引语暗角/删 TestController） | 编译/构建通过，**未跑浏览器 E2E** | 改动面小且不触及已验证路径；下次跑全量 E2E 时顺带覆盖 |
